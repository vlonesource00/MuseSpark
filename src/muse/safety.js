...
export class SafetySupervisor {
  ...
  update(car, cars, current, dt, t) {
    ...
    for (const o of cars) {
      ...
      if (ahead && dist < 9 && closing > 3.5) {
        this.emergency = closing > 7 && dist < 6;
      ...
      // Side overlap (door-to-door is TOLERATED — no intervention).
      if (dist < 3.4 && Math.abs(closing) < 6) {
        // shallow rub: do not intervene, just cap slightly.
        this.maxSpeed = Math.min(this.maxSpeed, car.speed + 1);
        if (this.reason === 'CLEAR') this.reason = 'CLEAR';
      }
    }
    // 2) Track departure.
